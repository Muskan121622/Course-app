...
//     window.addEventListener("storage", syncLogin); // When storage changes from another tab
...
//     checkLogin();
//   }, [location]); // 👈 This ensures check runs on each route navigation
...
//   // Check login every route change
//   useEffect(() => {
//     const user = sessionStorage.getItem("user");
//     setIsLoggedIn(!!user);
//   }, [location]);
...
//     const fetchCourses = async () => {
//       try {
//         const res = await axios.get("https://course-app-4-hb1g.onrender.com/api/v1/course/courses", {
//           withCredentials: true,
//         });
...
//   const handleLogout = async () => {
//     try {
//       const res = await axios.get("https://course-app-4-hb1g.onrender.com/api/v1/user/logout", {
//         withCredentials: true,
//       });
...
//                       <img className="h-32 w-full object-contain" src={course.image.url} alt={course.title} />
//                     ) : (
//                       <div className="h-32 w-full bg-gray-700 flex items-center justify-center text-white">No Image</div>
//                     )}
//                     <div className="p-6 text-center">
//                       <h2 className="text-xl font-bold text-white">{course.title}</h2>
//                       <Link to={`/buy/${course._id}`} className="mt-8 bg-orange-500 text-white py-2 px-4 rounded-full hover:bg-blue-500 duration-300">
  ...
  useEffect(() => {
    const handleTabClose = () => {
      if (localStorage.getItem("user")) {
        localStorage.removeItem("user");
        toast("You have been logged out automatically.");
      }
    };
